...
import { UserAvatar, type UserAvatarClassNames } from '@/components/features/avatar/user-avatar'
    ...
    <DropdownMenu>
      ...
      >
        {size === 'icon' ? (
          ...
          <Button className={cn('h-fit', className, classNames?.trigger?.base)} variant="outline">
            <User user={user} isPending={isPending} classNames={classNames?.trigger?.user} />
      ...
      >
        <div className={cn('p-2', classNames?.content?.menuItem)}>
          {(user && !user.isAnonymous) || isPending ? (
            <User user={user} isPending={isPending} classNames={classNames?.content?.user} />
          ) : (
