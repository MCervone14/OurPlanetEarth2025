    ...
    <Card className={cn('w-full overflow-hidden', className, classNames?.base)}>
      <div className="flex justify-between">
        <CardHeader className={cn('grow space-y-2', classNames?.header)}>
          <Skeleton className={cn('mt-0.5 h-5 w-24 md:mt-0 md:h-6', classNames?.skeleton)} />
        ...
        <div className="my-5 me-6">
          <Skeleton
            className={cn('size-20 rounded-full', classNames?.avatar?.base, classNames?.skeleton)}
          />
        </div>
      </div>
      ...
      >
        <Skeleton
          className={cn('mx-auto h-4 w-64 md:mx-0 md:my-0.5 md:w-80', classNames?.skeleton)}
        />
      </CardFooter>
    </Card>
  )
